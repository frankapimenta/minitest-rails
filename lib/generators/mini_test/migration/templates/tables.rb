...
<%- if migration_action == 'join' -%>
    def test_<%= file_name %>_table_schema
      ...
      assert !sql.table_exists?(:<%= join_tables.rotate.join('_') %>)      
    ...
    def test_<%= file_name %>_table_data
      ...
      sql.execute "INSERT INTO <%= join_tables.rotate.join('_') %> (<%= attributes.second.name  %>_id, <%= attributes.first.name %>_id) VALUES(\"#{_<%= attributes.second.name  %>_id }\", \"#{_<%= attributes.first.name %>_id }\")"

      _<%= join_tables.rotate.join('_') %>_row  = sql.select_one "SELECT <%= join_tables.first %>.* FROM <%= join_tables.first %>
      INNER JOIN <%= join_tables.rotate.join('_') %> ON <%= join_tables.rotate.join('_') %>.<%= attributes.first.name %>_id = <%= join_tables.first %>.id
      INNER JOIN <%= join_tables.second %> ON <%= join_tables.rotate.join('_')%>.<%= attributes.second.name %>_id = <%= join_tables.second %>.id
      WHERE <%= join_tables.first %>.id = \"#{_<%= attributes.first.name %>_id}\" AND <%= join_tables.second %>.id = \"#{_<%= attributes.second.name %>_id}\""

      assert_equal _<%= attributes.first.name %>_id,     _<%= join_tables.rotate.join('_') %>_row['id'].to_i
